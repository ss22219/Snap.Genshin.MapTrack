using Microsoft.VisualStudio.Threading;
using Snap.Core.DependencyInjection;
using System;
using System.Threading.Tasks;
using System.Windows;
using SystemPage = System.Windows.Controls.Page;

namespace Snap.Genshin.MapTrack
{
    [View(InjectAs.Transient)]
    public partial class MapPage : SystemPage, IDisposable
    {
        private const string MapUrl = "https://yuanshen.site/";
        private  const  string BrowserScript =  @"
const sleep = m => new Promise(r => setTimeout(r, m))
async function setPositionLoop(){
    while(true){
		if(window.L) await setPosition()
        await sleep(200)
    }
}

var playerMarket
const icon = 'data:image/png;base64,'
async function setPosition(){
	var result = await chrome.webview.hostObjects.Browser.GetPosition()
	if(!result || !result.length) return
	var pos = new L.LatLng(result[0], result[1])
	
	if(!playerMarket) {
    	playerMarket = L.marker([0, 0]).addTo(map)
	}
	
	playerMarket.setLatLng(pos)
	playerMarket.setZIndexOffset(1)
	playerMarket.setIcon(L.divIcon({iconSize: [50, 50], className:'', html:'<img src=""' + icon + '"" style=""width:50px;height:50px;transform: rotate(' + result[2] + 'deg);"">'}))
	
	map.panTo(pos)
}

(function() {
    setPositionLoop()
})()
";
        public MapPage()
        {
            this.InitializeComponent();
            BrowserInteractiveObject.init();
        }

        public void Dispose()
        {
            this.WebView?.Dispose();
            BrowserInteractiveObject.uninit();
        }
        
        private void WindowLoaded(object sender, RoutedEventArgs e)
        {         
            this.LoadMapAsync().Forget();
        }
        
        
        private async Task LoadMapAsync()
        {
            try
            {
                await this.WebView.EnsureCoreWebView2Async();
                this.WebView.CoreWebView2.ProcessFailed += (_, _) => this.WebView?.Dispose();
                this.WebView.CoreWebView2.AddHostObjectToScript("Browser", new BrowserInteractiveObject());
                await this.WebView.CoreWebView2.AddScriptToExecuteOnDocumentCreatedAsync(BrowserScript);
            }
            catch
            {
                this.Dispose();
                return;
            }

            this.WebView.Source = new Uri(MapUrl);
        }

        private void MapPage_OnUnloaded(object sender, RoutedEventArgs e)
        {
            this.Dispose();
        }
    }
}
